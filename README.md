...
Polls the GitHub API for repositories with a non-empty `## Unreleased` block in `CHANGELOG.md` and publishes a `CreateTaskCommand` to Kafka per affected repo so the [`github-releaser-agent`](https://github.com/bborbe/github-releaser-agent) picks each up and cuts the release autonomously.
...
Per the agent task-file contract — every emitted `CreateTaskCommand` carries this frontmatter shape:
...
The [`github-releaser-agent`](https://github.com/bborbe/github-releaser-agent) is
live and consumes these tasks. The slash-command `/github-release-repo` performs
the same release manually when a human wants to drive it.
